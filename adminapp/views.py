...
def category_update(request, pk):
    ...
    categiry_item = get_object_or_404(ProductCategory, pk=pk)

    if request.method == 'POST':
        edit_form = ProductCategoryEditForm(
            request.POST, request.FILES, isinstance=categiry_item)

        if edit_form.is_valid():
            edit_form.save()
            return HttpResponseRedirect(reverse('admin:category_update', args=[categiry_item.pk]))

    else:
        edit_form = ProductCategoryEditForm(isinstance=categiry_item)
